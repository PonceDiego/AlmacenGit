...
<body>
	...
	<!-- Page Content   -->
	<div class="container">
		<form class="form-signin" method="post" action="../EditarSubcategoria">
			<div class="card card-signin my-5">
				<div class="card-body">
					<h3 class="text-center">Datos de la subcategoría</h3>
					<div class="row">
						<div class="column">
							<div>Nombre</div>
							<div class="form-label-group>">
								<input type="text" name="inputNombre" id="inputNombre" value="${subcategoria.getNombre() }" class="form-control" placeholder="Nombre" required autocomplete="off" onchange="comparar()">
							</div>
							<div>Categoría</div>
							<div class=" form-label-group>" id="divCat">
								<input type="hidden" name="subcategoriaId" id="subcategoriaId" value="${subcategoria.getId() }"> 
								<input type="hidden" name="inputCat" id="inputCat" value="${subcategoria.getCategoria().getCategoriaId() }"> 
								<select onchange="selected()" id="selectCat" style="border-radius: 5px; font-size: 16px; padding: 10px;" required>
	...
	<script>
		$(document).ready(function() {
			var idCategoria = $("#inputCat").val();
			$("option[value=" + idCategoria + "]").attr('selected','selected');
			comparar();
		});
